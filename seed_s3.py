...
s3_endpoint_url = (
    "http://localhost:4566"  # Update this with your LocalStack endpoint if different
)
...
async def main(num_records: int):
    """
    Main function to generate mock data and upload to S3.

    Args:
    num_records (int): Number of pageview records to generate.
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upload pageview data to S3")
    parser.add_argument(
        "--num-records", type=int, default=100, help="Number of records to generate"
    )
    args = parser.parse_args()

    asyncio.run(main(args.num_records))
