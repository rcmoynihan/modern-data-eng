...
def write_to_s3(data: Dict, aggregated_key: str):
    ...
    try:
        ...
        with closing(boto3.client('s3', endpoint_url=AWS_ENDPOINT_URL, **AWS_CREDENTIALS)) as s3_client:
            s3_client.put_object(Bucket=staging_bucket, Key=aggregated_key, Body=json_data)
            logger.info(f"Successfully wrote aggregated data to {staging_bucket}/{aggregated_key}")
...
if __name__ == "__main__":
    lambda_handler({"latest_partition_key": "year=2024/month=01/day=01", "aggregated_key": "aggregated/2024-01-01.json"}, None)
